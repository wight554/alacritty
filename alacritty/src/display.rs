...
use alacritty_terminal::config::{Config, StartupMode};
...
impl Display<PossiblyCurrent> {
    ...
    pub fn handle_resize(&mut self, config: &Config, font_size: Size, size_info: SizeInfo) {
        if self.font_size == font_size && self.size_info == size_info {
            return;
        }
